  ...
  private buildInstruction(action: SessionAction): string {
    if (action === 'investigate') {
      return [
        ...
        '2. ユーザーと対話しながらアクションアイテムを整理する',
        '3. 整理したアクションを ACTIONS セクションに JSON 形式で書き込む',
        '4. 補足説明や要約は DRAFT セクションに書く',
        '',
        '■ ACTIONS セクションの書き方:',
        '既存の <!-- ACTIONS [...] --> タグを Edit ツールで置換する。タグがなければ <!-- DRAFT --> の直前に挿入する。',
        'フォーマット: <!-- ACTIONS [JSON配列] -->',
        '',
        '各アクションアイテムの JSON 構造:',
        '{',
        '  "id": "ランダムな一意ID (例: act-1, act-2)",',
        '  "type": "create-issue" | "backlog-comment" | "create-document" | "todo",',
        '  "status": "draft",',
        '  "title": "アクションのタイトル",',
        '  "content": "本文（課題の説明、コメント内容、ドキュメント本文など）",',
        '  "issueKey": "BNN-123 (backlog-comment の場合、対象課題キー)",',
        '  "documentName": "ドキュメント名 (create-document の場合)"',
        '}',
        '',
        '■ type の使い分け:',
        '- create-issue: 新しい Backlog 課題を作成する場合',
        '- backlog-comment: 既存課題にコメントを追加する場合（issueKey 必須）',
        '- create-document: Backlog ドキュメントやまとめを作成する場合',
        '- todo: 上記以外の作業メモ（確認待ち、調査など）',
        '',
        '■ ルール:',
        '- ユーザーとの対話で内容を確認しながら進める',
        '- CONTEXT セクションと INSTRUCTION セクションは編集しない',
        '- ACTIONS タグは 1 行で書く（改行を含めない）',
        '/INSTRUCTION -->',
      ].join('\n');
    }
