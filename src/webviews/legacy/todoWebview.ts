  ...
  <script nonce="${nonce}">
    ...
    window.addEventListener('message', (event) => {
      ...
      if (msg.command === 'chatTurnStart') {
        ...
        document.getElementById('claudeStopBtn').style.display = 'inline-block';
        document.getElementById('chatSendBtn').disabled = true;
        window._currentAssistantBubble = bubble;
      }
      if (msg.command === 'chatChunk') {
        if (window._currentAssistantBubble) {
          window._currentAssistantBubble.textContent = msg.text;
          const messages = document.getElementById('chatMessages');
          messages.scrollTop = messages.scrollHeight;
        }
      }
      if (msg.command === 'chatDone') {
        window._currentAssistantBubble = null;
        document.getElementById('claudeStopBtn').style.display = 'none';
      ...
      if (msg.command === 'chatError') {
        const messages = document.getElementById('chatMessages');
        if (window._currentAssistantBubble) {
          window._currentAssistantBubble.className = 'chat-msg error';
          window._currentAssistantBubble.textContent = 'Error: ' + msg.text;
        } else {
    ...
    function sendChatMessage() {
      ...
      input.value = '';
      input.style.height = '';
      vscode.postMessage({ command: 'sendChatMessage', text, model });
    }
    ...
    // Stop / close chat
    document.getElementById('claudeStopBtn')?.addEventListener('click', () => {
